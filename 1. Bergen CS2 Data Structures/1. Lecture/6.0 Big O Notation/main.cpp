...
O(n^3) "Order n cubed" (Cubic time). Can be calculated with a simple equation. See below.
for (i in [N])
    if (j in [M])
        if (i == j)
            cout << j;
This would result in T(n) = O(1) + O(1) + O(M) * O(n)
T(n) = O(m*n) --> T(n) = O(m^2) (M and N are the same)

O(2^n) "Order 2 to the n" (Exponential time). You do NOT want exponential increases. Recursion is an example of this.
